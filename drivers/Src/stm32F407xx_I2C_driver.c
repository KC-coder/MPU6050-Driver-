...
/*
 * instructor code
 * buggi , found problem when reciving data from MPU6050 sensor
 * the code is not per the guidelines provided in the datasheet of the STM32's I2C peripheral
 * if we use this the the master(here our STM32F4 device sends ACK at the last byte reception because of
 * which the slave does't let go the SDA line and as it is last line the master doe'st produce clk
 * as slave is holding the line  because of which the stop conditon does't get generated
 *  and the SDA line remains low, it will cause problem in further communication
 *  as SDA is low so now no start condition can be generated
 * if(Len == 1)
	{
		//Disable Acking
		I2C_ManageAcking(pI2CHandle->pI2Cx , I2C_ACK_DISABLE);
 ...
 * @Note              -  Interrupt handling for different I2C events (refer SR1)
						...
						in this API to check which event is  causing the interrupt
						we check what flag is set and is the control bit for that interrupt event is enabled
						or not
...
void I2C_EV_IRQHandling(I2C_Handle_t *pI2CHandle)
{
	...
	temp3 = pI2CHandle->pI2Cx->SR1 & (1 << I2C_SR1_SB);////to store the value of SB in temp2 variable
